...
SR_STOP = os.path.join(CP_DIR, "bin", "schema-registry-stop")
...
class ConfluentSchemaRegistryService(Service):
    ...
    def _properties(self) -> str:
        return (
            ...
            # Single-node, replication of 1 is required when the source
            # cluster has only one broker.
    ...
    def pids(self, node):
        return node.account.java_pids("schemaregistry.SchemaRegistryMain")
