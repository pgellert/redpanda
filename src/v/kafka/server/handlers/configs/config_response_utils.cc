...
static void add_broker_config(
  ...
  Func&& describe_f) {
    ...
    result.push_back(config_response_t{
      .name = ss::sstring(name),
      .value = describe_f(property.value()),
      .config_source = src,
...
describe_configs_resource_result config_response_t::to_describe_config() {
    return describe_configs_resource_result{
      .name = std::move(name),
...
creatable_topic_configs config_response_t::to_create_config() {
