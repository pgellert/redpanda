...
// TODO: move to taking context_schema_id by reference
...
public:
    ...
    // TODO: this is only used for testing, so replace with the test store
    // fixture instead insert_result insert(subject_schema schema) {
    ...
    get_schema_definition(const context_schema_id& id) const {
        ...
        if (it == _schemas.end()) {
            // TODO: double-check error message includes context
            ...
            return std::ranges::any_of(
              s.second.versions, [&ids, &s, inc_del](const auto& v) {
                  return (inc_del || !s.second.deleted)
                         && ids.contains(context_schema_id{s.first.ctx, v.id});
              });
        });
    }
    ...
    result<mode> get_mode(const context& ctx) const {
        // TODO: consider whether we need to throw if ctx is not found
        return _context_stores.contains(ctx) ? _context_stores.at(ctx)._mode
                                             : mode::read_write;
    }
    ...
    result<bool> set_mode(const context& ctx, mode m, force f) {
        ...
        // TODO: consider whether we need to create the context entry / throw if
        // ctx is not found
    ...
    result<compatibility_level> get_compatibility(const context& ctx) const {
        // TODO: consider whether we need to throw if ctx is not found
        return _context_stores.contains(ctx)
                 ? _context_stores.at(ctx)._compatibility
    ...
    void maybe_update_max_schema_id(const context_schema_id& id) {
        // TODO: maybe throw if the context doesn't exist?
...
private:
    ...
    private:
        ...
        void setup_metrics(const context_subject& sub) {
            // TODO: figure out how to incorporate the context label here
            namespace sm = ss::metrics;
            auto group_name = prometheus_sanitize::metrics_name(
              "schema_registry_cache");
