...
post_subject_versions(server::request_t rq, server::reply_t rp) {
    ...
    vlog(srlog.debug, "post_subject_versions: existing ID {}", s_id);
    ...
    const auto id_matches
      = s_id.has_value()
        && (schema.id == invalid_schema_id || schema.id == s_id);
    const auto version_matches
      = v_id.has_value()
        && (schema.version == invalid_schema_version || schema.version == v_id);
    const auto exists = id_matches && version_matches;
