...
// CORE-16246 regression: the verbose and bare forms reported in the ticket
// must canonicalize to the same definition.
...
// A primitive object carrying additional attributes (e.g. logicalType) is
// NOT a primitive simple-form equivalent and must be preserved.
...
// not equivalent to the bare string form and must be preserved.
...
// Sanitization is idempotent: a canonical schema must be a fixed point.
BOOST_AUTO_TEST_CASE(test_sanitize_avro_idempotent) {
...
// Custom metadata members on array/map/enum/fixed schemas may contain
// arbitrary JSON values, including ones that incidentally have the shape of
// a primitive schema (e.g. {"type":"int"}). Sanitize must not recurse into
// such custom members, or the primitive-object collapse would corrupt them.
const pps::schema_definition custom_metadata_in_complex_types{
  R"({"type":"record","name":"R","fields":[)"
  R"({"name":"a","type":{"type":"array","items":"string",)"
  R"("custom":{"type":"int"}}},)"
  R"({"name":"m","type":{"type":"map","values":"string",)"
  R"("custom":{"type":"int"}}},)"
  R"({"name":"e","type":{"type":"enum","name":"E","symbols":["x","y"],)"
  R"("custom":{"type":"int"}}},)"
...
BOOST_AUTO_TEST_CASE(
  test_sanitize_avro_preserves_complex_type_custom_metadata) {
    BOOST_REQUIRE_EQUAL(
      pps::sanitize_avro_schema_definition(
        custom_metadata_in_complex_types.share())
        .value(),
