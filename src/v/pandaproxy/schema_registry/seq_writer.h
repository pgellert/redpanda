...
public:
    ...
    ss::future<> check_mutable(const context& ctx, const std::optional<subject>& sub);
    ...
    ss::future<bool> delete_config(context_subject sub);

    ss::future<bool>
    write_mode(context ctx, std::optional<subject> sub, mode m, force f);

    ss::future<bool> delete_mode(context_subject sub);
...
private:
    ...
    ss::future<std::optional<bool>> do_delete_config(context_subject sub);
    ...
    do_delete_mode(context_subject sub, model::offset write_at);
