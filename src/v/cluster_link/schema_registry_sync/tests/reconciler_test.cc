...
TEST(reconciler, abort_mid_sync_drains_cleanly) {
    ...
    srs::reconciler::limits lim{.memory_bytes = 1u << 20, .parallelism = 4};
    ...
    // The units held across the blocked import were released on the abort path
    // (no leak): availability is back to the full budget.
    EXPECT_EQ(r.available_memory_units(), lim.memory_bytes);
}
...
TEST(reconciler, concurrent_stress) {
    ...
    h.lim = srs::reconciler::limits{.memory_bytes = 64, .parallelism = 4};
    ...
    // Every byte-budget unit was released: no leak.
    EXPECT_EQ(r.available_memory_units(), 64u);
