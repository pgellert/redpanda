...
- Ducktape coverage for the `https://` proxy URL path. v1 ducktape only exercises plaintext proxy (`http://mitmproxy:8888`), matching the customer scenario. The `https://` proxy code path (TLS handshake to the proxy before CONNECT, nested TLS) is exercised by any consumer in code but not end-to-end tested. Would require a TLS-listener in front of mitmproxy (e.g. stunnel with a test cert) plus installing the test CA into each Redpanda node's system trust store before broker start. Deferred as a separate follow-up to avoid bloating the MVP scope.
